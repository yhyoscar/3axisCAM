...
(function () {
    ...
    jscut.priv.cam.outline = function (geometry, cutterDia, isInside, width, overlap, climb) {
        ...
        if (isInside) {
            ...
            //('before diff:', current, bounds, bounds, eachOffset, needReverse);
            bounds = jscut.priv.path.diff(current, jscut.priv.path.offset(geometry, -(width - cutterDia / 2)));
            eachOffset = -eachWidth;
            needReverse = climb;
            //console.log('after diff:', current, bounds, bounds, eachOffset, needReverse);
        } else {
            current = jscut.priv.path.offset(geometry, cutterDia / 2);
    ...
    jscut.priv.cam.engrave = function (geometry, climb) {
        ...
        for (var i = 0; i < geometry.length; ++i) {
            var tmp = geometry[i].slice(0);        
            if (!climb)
                tmp.reverse();

            //path.push(path[0]);
            allPaths.push(tmp);
            //console.log('path:', i, geometry[i].length, tmp.length, geometry[i], tmp);
        }

        // for (var i=0; i<allPaths.length; i++){
        //     //console.log('allpath:', i, allPaths[i].length, allPaths[i]);
        // }

        var result = mergePaths(null, allPaths);

        for (var i = 0; i < result.length; ++i)
            result[i].safeToClose = false;
        return result;
    };
