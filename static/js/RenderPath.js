...
function startRenderPathDemo() {
    ...
    renderPath = startRenderPath({}, $("#renderPathCanvas")[0], $('#timeSlider'), 'js', function (renderPath) {
        $.get("logo-gcode.txt", function (gcode) {
            ...
            renderPath.fillPathBuffer(jscut.parseGcode({}, gcode), 0, 3.0, 180, 10);
        });
    });
}

function render_gcode(gcode, ztop, cutd, cuta, cuth) {
    var renderPath;
    var num_ztop = parseFloat(ztop);
    var num_cutd = parseFloat(cutd);
    var num_cuta = parseFloat(cuta);
    var num_cuth = parseFloat(cuth);   
    renderPath = startRenderPath({}, $("#renderPathCanvas")[0], $('#timeSlider'), 'js', function (renderPath) {                 
        renderPath.fillPathBuffer(jscut.parseGcode({}, gcode), num_ztop, num_cutd, num_cuta, num_cuth);
    });
}
